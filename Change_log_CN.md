...
## v3.140.01
- ★ 深色模式 ☆
- 现在您可以在“主页”中切换浅色/深色模式外观。
- 新增：[保留源图像的 ICC]。保存图片时将保留 ICC 色彩配置文件。
- 部分图片的放大后色彩准确度会更好。
- 新增：可通过右键菜单快速访问日志文件夹。
- 问题修复：
- 修复了开启“图像批量放大”时的 RTX 50 性能冲突问题。
- 修复了 VRAM 读数，现提供更准确的数值与更好的兼容性。
- 修复了 SRMD-CUDA 将带有 Alpha 通道的图像路由至错误引擎的问题。
- 修复了部分保存失败的图像被错误标记为“已完成”的问题。
- 批量处理期间，源文件将获得更好的保护。
- 提升了批量处理的容错能力。
- 修复了核心批量计算逻辑，提高了空间预估准确度与任务分组效率。
- 改进了文件操作兼容性，带来更可靠的路径处理与重复文件名处理。
- 修复了若干微小的线程与缓存问题。
---
## v3.139.01
