...
\n\nWe recommend that you install this update after finish processing all the Video in the
Files list.This version is not compatible with the cache generated by the old version of
 the software.\n\n建议您在处理完文件列表内的所有视频后再安装本次更新,因为本次更新与旧版本生成的缓存文件不兼容.
